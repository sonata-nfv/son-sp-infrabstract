...
public class WrapperFactory {
	...
	private static ComputeWrapper createComputeWrapper(WrapperConfiguration config){
		...
		if(config.getVimType().equals(ComputeVimType.OPENSTACKHEAT.getName()))
			output = new OpenStackHeatWrapper(config);
		
		//TODO Extends with all wrappers or refactor with a more OO type generation
		
		return output;
	}
	
	private static NetworkingWrapper createNetworkingWrapper(WrapperConfiguration config){return null;}
	private static StorageWrapper createStorageWrapper(WrapperConfiguration config){return null;}
}
